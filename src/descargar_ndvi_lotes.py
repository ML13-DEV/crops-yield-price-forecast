...
def descargar_ndvi_punto(task_id, token):
    ...
    columnas_ndvi = [c for c in df.columns if "ndvi" in c.lower()]
    if not columnas_ndvi:
        raise ValueError(
            f"No se encontró ninguna columna de NDVI en el CSV de AppEEARS. "
            f"Columnas disponibles: {df.columns.tolist()}"
        )
    columna_ndvi = columnas_ndvi[0]
...
def main():
    """Descarga los resultados de los requests ya enviados para cada lote."""
    ruta_ids = Path("ndvi_lotes_task_ids.json")
    ruta_salida = Path("ndvi_lotes")

    if not ruta_ids.exists():
        raise FileNotFoundError(
            f"No se encontró {ruta_ids}. Ejecutá primero enviar_ndvi_lotes.py."
        )

    with ruta_ids.open("r", encoding="utf-8") as archivo:
        task_ids = json.load(archivo)

    usuario = input("Usuario Earthdata Login: ")
    contrasena = getpass.getpass("Contraseña: ")
    respuesta = requests.post(
        f"{API}login", auth=(usuario, contrasena), timeout=120
    )
    ...
    descargados = 0
    pendientes = 0
    for lote_id, task_id in task_ids.items():
        ruta_csv = ruta_salida / f"lote_{lote_id}.csv"
        if ruta_csv.exists():
            print(f"Lote {lote_id}: ya descargado, salteando.")
            descargados += 1
            continue

        df_ndvi = descargar_ndvi_punto(task_id, token)
        if df_ndvi is None:
            pendientes += 1
            continue

        df_ndvi.to_csv(ruta_csv, index=False)
        descargados += 1
        print(f"Lote {lote_id}: resultado guardado en {ruta_csv}.")

    print(
        f"\nDescargados: {descargados} | Pendientes: {pendientes} "
        f"| Total: {len(task_ids)}."
    )
