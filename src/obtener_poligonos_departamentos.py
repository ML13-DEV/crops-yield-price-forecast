...
def filtrar_departamentos(geojson_completo, ids_departamentos):
    ...
    """
    # correcciones conocidas: casos donde el id de MAGyP no coincide con
    # el id oficial de georef/INDEC para el mismo departamento.
    CORRECCIONES_ID = {
        '06217': '06218',  # Chascomús, Buenos Aires
    }

    ids_set = set(str(int(float(i))).zfill(5) for i in ids_departamentos)
    ids_set = {CORRECCIONES_ID.get(i, i) for i in ids_set}
...
if __name__ == "__main__":
    ...
    deptos_filtrados = pd.read_csv(r"..\data\processed\departamentos_filtrados.csv")
